...
def _paper_context(result: Mapping[str, Any], output_dir: Path, *, title: str | None, config: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "figures": figures,
        "tables": [],
        "evidence_candidate": _load_evidence_candidate(output_dir),
...
def _load_evidence_candidate(output_dir: Path) -> dict[str, Any]:
    report_path = output_dir / "data_import_v30" / "evidence_package_vv_candidate.json"
    if not report_path.exists():
        return {
            "存在": False,
            "候选门槛满足": False,
            "候选评分状态": "尚未生成证据包 V&V 候选评分",
            "输出文件": str(report_path.resolve()),
            "正式评分策略": {"是否改写正式评分": False, "候选门槛满足": False},
            "关键指标": {},
            "风险信号": ["尚未生成证据包 V&V 候选评分报告。"],
            "安全边界": "证据包候选评分只作为论文证据链附注，不输出真实作用距离、器件阈值或毁伤概率。",
        }
    try:
        ...
        return {
            "存在": False,
            "候选门槛满足": False,
            "候选评分状态": "证据包 V&V 候选评分报告读取失败",
            "输出文件": str(report_path.resolve()),
            "正式评分策略": {"是否改写正式评分": False, "候选门槛满足": False},
            "关键指标": {},
            "风险信号": [f"证据包候选评分报告读取失败：{exc}"],
            "安全边界": "证据包候选评分读取失败时不进入正式评分或论文结论。",
        }
    if not isinstance(payload, dict):
        payload = {"候选评分状态": "证据包 V&V 候选评分报告不是 JSON 映射"}
    payload = dict(payload)
    payload["存在"] = True
    payload.setdefault("输出文件", str(report_path.resolve()))
    payload.setdefault("正式评分策略", {"是否改写正式评分": False, "候选门槛满足": False})
    payload.setdefault("风险信号", [])
    payload.setdefault("关键指标", {})
    payload.setdefault("候选门槛满足", bool(_as_mapping(payload.get("正式评分策略")).get("候选门槛满足")))
    return payload


def _candidate_report_path(candidate: Mapping[str, Any], output_dir: Path) -> Path:
    value = candidate.get("输出文件")
    if isinstance(value, str) and value.strip():
        path = Path(value)
        return path if path.is_absolute() else output_dir / path
    return output_dir / "data_import_v30" / "evidence_package_vv_candidate.json"


def _candidate_risk_signals(candidate: Mapping[str, Any]) -> list[str]:
    signals = candidate.get("风险信号", [])
    if isinstance(signals, str):
        signals = [signals]
    return [str(item) for item in signals if str(item).strip()] or ["未发现证据包候选评分风险信号。"]


def _candidate_manifest_summary(candidate: Mapping[str, Any], output_dir: Path) -> dict[str, Any]:
    path = _candidate_report_path(candidate, output_dir)
    metrics = _as_mapping(candidate.get("关键指标"))
    strategy = _as_mapping(candidate.get("正式评分策略"))
    return {
        "存在": bool(candidate.get("存在")) and path.exists(),
        "输出文件": str(path.resolve()),
        "候选门槛满足": bool(candidate.get("候选门槛满足")),
        "候选评分状态": candidate.get("候选评分状态", ""),
        "正式评分改写": bool(strategy.get("是否改写正式评分")),
        "正式评分策略说明": strategy.get("说明", ""),
        "关键指标": {
            "标定后相对RMSE/%": metrics.get("标定后相对RMSE/%"),
            "2sigma覆盖率/%": metrics.get("2sigma覆盖率/%"),
            "中位归一化残差": metrics.get("中位归一化残差"),
            "标定后P95残差": metrics.get("标定后P95残差"),
        },
        "风险信号": _candidate_risk_signals(candidate)[:8],
        "安全边界": candidate.get("安全边界", ""),
    }
...
def _reproduction_registry_rows(config: Mapping[str, Any], context: Mapping[str, Any], output_dir: Path) -> list[dict[str, Any]]:
    ...
    candidate = _as_mapping(context.get("evidence_candidate"))
    candidate_path = _candidate_report_path(candidate, output_dir)
    candidate_exists = bool(candidate.get("存在")) and candidate_path.exists()
    candidate_ready = bool(candidate.get("候选门槛满足"))
    if candidate_ready:
        candidate_status = "候选门槛通过，待人工复核"
    elif candidate_exists:
        candidate_status = "候选门槛未通过，仅作风险附注"
    else:
        candidate_status = "尚未生成候选评分报告"
    rows.append(
        {
            "编号": "REP-DATA-001",
            "类型": "external_data_candidate",
            "主题": "证据包 V&V 候选评分",
            "引用键": "hpm_dt_evidence_candidate",
            "证据": str(candidate_path.resolve()),
            "证据存在": candidate_exists,
            "状态": candidate_status,
            "阻断项": "；".join(_candidate_risk_signals(candidate)[:3]),
        }
    )
    return rows
...
def _statistics_audit(context: Mapping[str, Any], config: Mapping[str, Any]) -> dict[str, Any]:
    ...
    candidate = _as_mapping(context.get("evidence_candidate"))
    candidate_summary = _candidate_manifest_summary(candidate, output_dir)
    candidate_strategy = _as_mapping(candidate.get("正式评分策略"))
    candidate_checks = [
        _audit_check("证据包候选评分报告存在", candidate_summary["存在"], candidate_summary["输出文件"]),
        _audit_check("证据包候选不自动改写评分", candidate_strategy.get("是否改写正式评分") is False, candidate_strategy.get("说明", "")),
        _audit_check("证据包候选门槛满足", candidate_summary["候选门槛满足"], candidate_summary["候选评分状态"]),
    ]
    checks = [
    ...
    return {
        ...
        "关键指标": {
            ...
            "证据包候选门槛满足": candidate_summary["候选门槛满足"],
            "证据包候选正式评分改写": candidate_summary["正式评分改写"],
        },
        "检查项": checks,
        "证据包候选评分审计": {
            "要求报告存在": bool(stats_cfg.get("require_evidence_candidate", True)),
            "报告存在": candidate_summary["存在"],
            "候选门槛满足": candidate_summary["候选门槛满足"],
            "不自动改写正式评分": candidate_strategy.get("是否改写正式评分") is False,
            "候选评分状态": candidate_summary["候选评分状态"],
            "关键指标": candidate_summary["关键指标"],
            "风险信号": candidate_summary["风险信号"],
            "检查项": candidate_checks,
            "说明": "候选门槛未满足时只作为论文风险附注，不参与 Paper Factory 总体验收失败判定。",
        },
...
def _markdown_draft(context: Mapping[str, Any]) -> str:
    ...
    candidate = _as_mapping(context.get("evidence_candidate"))
    candidate_summary = _candidate_manifest_summary(candidate, output_dir)
    candidate_metrics = candidate_summary["关键指标"]
    candidate_risks = "\n".join(f"- {item}" for item in candidate_summary["风险信号"][:6])
...
外部数据证据包候选评分已经接入论文证据链，用于记录每阵元功率元数据、实测标定点、源链、相位参考、残差和不确定度覆盖门槛。该报告不自动改写正式可信度评分，也不输出真实作用距离、器件阈值或毁伤概率。

- 候选报告存在：{_yes_no(candidate_summary['存在'])}
- 候选门槛满足：{_yes_no(candidate_summary['候选门槛满足'])}
- 候选评分状态：{candidate_summary['候选评分状态']}
- 标定后相对RMSE/%：{candidate_metrics.get('标定后相对RMSE/%', 'NA')}
- 2sigma覆盖率/%：{candidate_metrics.get('2sigma覆盖率/%', 'NA')}
- 是否改写正式评分：{_yes_no(candidate_summary['正式评分改写'])}
- 候选报告路径：`{candidate_summary['输出文件']}`
...
- 报告：`{candidate_summary['输出文件']}`
- 报告存在：{_yes_no(candidate_summary['存在'])}
- 候选门槛满足：{_yes_no(candidate_summary['候选门槛满足'])}
- 候选评分状态：{candidate_summary['候选评分状态']}
- 标定后相对RMSE/%：{candidate_metrics.get('标定后相对RMSE/%', 'NA')}
- 2sigma覆盖率/%：{candidate_metrics.get('2sigma覆盖率/%', 'NA')}
...
- 证据包 V&V 候选评分：`tables/*evidence_package_vv_candidate.json`
...
def _acceptance(
    *,
    ...
    latex_text = latex_path.read_text(encoding="utf-8") if latex_path.exists() else ""
    ...
    return [
        ...
        {"项目": "IEEE LaTeX 骨架", "通过": latex_path.exists() and "\\documentclass" in latex_text},
        {"项目": "引用库", "通过": bibliography_path.exists() and "@misc" in bibliography_text},
