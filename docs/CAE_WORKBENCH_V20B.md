...
V2.0B 的目标是把 HPM-DT 的 Workbench Layer 从表单式参数配置推进到三维工程建模。当前版本先完成一个可运行、可测试、可继续扩展的最小闭环：后端从 `CAEProject` 生成三维场景 JSON，前端用本地 Three.js 渲染场景，对象和材料属性修改必须回到后端通过工程约束校验后才会更新，材料代理审计会把引用完整性、字段白名单、参数范围和安全边界写入 `MAT-AUDIT-001`；当前三维工程状态可送入快速 CAE 求解器生成任务记录、结果摘要、结果图层和可复查结果档案；右侧属性面板新增绝对量纲标定卡片，可记录每阵元输入功率元数据和实测标定点，并输出校准残差与不确定度；求解任务支持本地取消请求、暂停检查点、恢复执行、重试派生和操作审计；工程快照可以落盘、恢复并对比对象/材料字段差异；统一工程资产台账把 `JOB-xxxx`、`SOL-xxxx`、`SNP-xxxx`、`IMP-CAL-001` 和 `MAT-AUDIT-001` 汇总成 JSON、CSV、SQLite 与审计报告轻量索引，把求解任务操作日志同步写入 SQLite 事件表，并提供资产谱系追溯、可复现实验审计、资产/快照命名规范审计。

V2.0B 的产品入口已经调整为 Scene First：Web 首页默认显示“场景编辑”，首屏围绕 `场景 / 对象 / 任务 / 结果` 组织三维视口、对象树、属性面板、求解按钮和结果档案；“可信度验证”保留为 `工具 -> 可信度验证`，用于发文、报告和验收复核。
...
- `/api/workbench3d/materials/audit` 输出材料代理审计，检查材料引用完整性、可编辑字段白名单、参数范围和安全边界，并以 `MAT-AUDIT-001` 纳入资产台账；
- `/api/workbench3d/absolute-calibration` 支持每阵元输入功率元数据、实测标定点、校准系数、残差、2σ覆盖率和实测距离覆盖区间的读写与落盘；V3.0 `/api/data-import/evidence-package/template` 会生成同字段的证据包模板，便于把 Workbench 绝对标定记录纳入授权数据证据链；
...
- `/api/workbench3d/assets`、`/api/workbench3d/assets/audit`、`/api/workbench3d/assets/database`、`/api/workbench3d/assets/database/records`、`/api/workbench3d/assets/lineage`、`/api/workbench3d/assets/reproducibility`、`/api/workbench3d/assets/imported-calibration`、`/api/workbench3d/assets/naming` 和 `/api/workbench3d/assets/{asset_id}` 支持求解任务、求解结果、工程快照、导入标定桥接和材料代理审计的统一资产台账、筛选、审计、SQLite 数据库审计、库表浏览、资产谱系追溯、可复现实验审计、命名规范审计与详情跳转；
...
- 统一工程资产台账汇总求解任务、求解结果、工程快照、导入标定桥接和材料代理审计，输出 `workbench3d_assets/index.json`、`index.csv`、`assets.sqlite`、`audit.json`、`audit.csv`、`database_audit.json`、`database_audit.csv`、`lineage.json`、`lineage.csv`、`reproducibility_audit.json`、`reproducibility_audit.csv`、`absolute_calibration.json`、`absolute_calibration_points.csv`、`absolute_element_powers.csv`、`imported_calibration_bridge.json`、`imported_calibration_bridge.csv`、`material_proxy_audit.json`、`material_proxy_audit.csv`、`naming_audit.json` 与 `naming_audit.csv`；SQLite 同时维护 `workbench3d_assets`、`workbench3d_solve_jobs`、`workbench3d_solve_job_events`、`workbench3d_results`、`workbench3d_snapshots` 和 `workbench3d_database_manifest` 表，前端可按类型/关键字筛选资产、触发审计、检查数据库事件行数、浏览任务/结果/快照库表、查看快照-任务-结果-重试的资产谱系、检查导入数据标定桥接 `IMP-CAL-001`、检查材料代理审计 `MAT-AUDIT-001`、检查结果 JSON/哈希/JOB/SOL/V&V 记录可复查性、检查 `JOB/SOL/SNP` 编号和文件名哈希绑定，并从台账入口跳转到任务、结果、快照、导入桥接或材料审计详情；
- 撤销/重做历史栈记录受控编辑；
...
这是 V2.0B 的工程原型，不是完整 CAD/CAE 建模器。当前编辑闭环支持属性面板、材料设置、材料代理审计、绝对量纲标定、导入数据标定桥接、三维拖拽移动、受控几何变换控件、快速求解联动、求解任务队列、本地后台worker预览、任务暂停/恢复/取消/重试/审计、SQLite 任务事件表、观察面结果图层回写、V&V 适用性诊断、对象指标列表/三维徽标、求解结果档案、工程快照落盘/恢复/差异对比和统一工程资产台账筛选/审计/库表浏览/谱系追溯/复现审计/命名规范检查，三维视口支持选择、旋转、缩放、重置视图和 JSON 快照导出，求解面板可读出色标、峰值坐标、中心剖面采样、x/y 剖面交互曲线和对象级约束状态。材料编辑器和材料代理审计仅处理归一化材料代理参数，不等价于全波材料库、真实器件阈值或失效预测。求解联动复用平台已有波长尺度归一化快速求解器，不是 CST/HFSS/COMSOL 级全波求解。当前后台worker仍是单机本地线程预览，只服务归一化快速求解检查点；SQLite 仍是单机轻量数据库，资产谱系只表达归一化快照、JOB、SOL、导入桥接资产、材料审计资产和本地事件日志关系，不代表真实设备状态继承；绝对量纲标定只表达用户输入功率元数据与实测点残差，V3.0 证据包模板把这些记录整理为可授权、可哈希复查的 manifest/CSV 包，但仍不代表真实作用距离或器件阈值；导入数据桥接只表达外部样本、坐标归一化、代理模型残差和不确定度证据；资产命名规范仍是本地台账级审计而非工厂级数据库主键策略，几何变换控件仍是按钮式受控编辑而非完整三维拖拽 Gizmo，正式多用户调度器和正式工程数据库仍属于后续门槛。
