...
PLATFORM_NORTH_STAR = (
    ...
    "效应分析与数字孪生研究的全中文科研级 CAE 平台。所有版本开发都必须服务于这个长期目标，"
    "而不是孤立地增加功能。"
)
...
PLATFORM_LAYER_STATUS = (
    {
        ...
        "当前证据": "已有 RectangularArray、自由空间/镜像/孔缝腔体/混合传播后端、材料/几何对象，以及 CST/HFSS 导出包、测量批次、CSV/Touchstone/NPZ/HDF5 外部数据导入审计层、CalibrationSamples 桥接预览、代理模型误差对比预览和外部数据 V&V 预评分审计。",
        "下一步": "把外部数据误差对比预览推进到真实源链/相位参考接入、正式可信度评分纳入和三维工作台结果查看。",
    },
    {
        "层级": "感知层",
        "英文": "Perception Layer",
    ...
    },
    {
        ...
        "当前状态": "V2.0A 已完成",
        "当前证据": "已有 6 类自动 V&V、Monte Carlo、不确定度、敏感性、可信度评分，以及外部数据残差/不确定度预评分审计。",
        "下一步": "补文献复现注册表、真实源链/相位参考和真实数据导入后的正式评分纳入。",
    },
    {
        "层级": "CAE平台层",
        "英文": "Workbench Layer",
        "职责": "项目、建模、求解、结果、报告、数据库",
        "当前状态": "V2.0B 三维原型与 V2.0C 插件市场接入中",
        "当前证据": "已有 FastAPI + Bootstrap 工作台、V2.0B Three.js 本地三维视口、对象树、属性面板、材料设置、拖拽移动、求解联动、观察面结果图层回写、色标与中心剖面读数、求解结果档案、轻量结果索引、撤销重做、工程快照落盘恢复、受控更新 API，以及 V2.0C 本地插件目录、启停、白名单运行和验收 API。",
        "下一步": "补尺寸/旋转 Gizmo、快照差异对比、求解任务队列、正式工程/结果数据库管理和更多插件类型。",
    },
    {
        "层级": "论文生产层",
        ...
        "当前证据": "已有 HTML 报告、论文图包、LaTeX 表格、论文提纲、Notebook，以及 V2.0D 自动论文草稿、IEEE骨架、图表清单、补充材料索引和论文包 ZIP。",
        "下一步": "补多模板、统计显著性报告、引用库、期刊模板和可编译检查。",
    },
)
...
PLATFORM_MILESTONE_STATUS = (
    ...
    },
    {
        ...
        "名称": "真正三维 CAE 编辑器",
        "状态": "进行中",
        "验收证据": "已接入 Three.js 本地视口、对象树、属性面板、材料设置、拖拽移动、求解联动、观察面结果图层回写、色标与中心剖面读数、求解结果档案、轻量结果索引、撤销重做、工程快照落盘恢复和后端几何/材料校验 API。",
        "下一门槛": "完成尺寸/旋转 Gizmo、快照差异对比、正式工程/结果数据库管理和求解任务队列验收。",
    },
    {
        "阶段": "V2.0C",
        "名称": "插件系统与 Plugin Marketplace",
        "状态": "预览接入中",
        "验收证据": "已有本地 JSON manifest、语义版本校验、启停 API、参数 Schema、白名单 builtin_hook、插件市场页面和三类内置插件运行审计。",
        "下一门槛": "补插件级 V&V、外部插件包签名/沙箱、保护/控场插件和工作台求解器联动。",
    },
    {
        "阶段": "V2.0D",
        "名称": "论文自动生产线 Paper Factory",
        "状态": "预览接入中",
        "验收证据": "已有 PaperFactoryService、生成 API、下载端点、UI 入口、Markdown 论文草稿、IEEE LaTeX 骨架、图表清单、补充材料索引和可复现论文包 ZIP。",
        "下一门槛": "补实验设计器、统计显著性报告、引用库、模板选择和 LaTeX 编译验收。",
    },
    {
        "阶段": "V3.0",
        "名称": "真实数据接入",
        "状态": "预览接入中",
        "验收证据": "已有 DataImportService、CST/HFSS 导出包识别、Measurement Campaign 测量批次识别、CSV/Touchstone/NPZ/HDF5 格式识别、单位/坐标列审计、工程元数据预览、仪器链/校准/不确定度审计、mm 到 lambda 坐标规范化预览、标定准备度报告、CalibrationSamples 标定桥接预览、代理模型误差对比预览、外部数据 V&V 预评分审计、样例目录、路径解析 API 和工作台数据导入页面。",
        "下一门槛": "把代理误差对比预览推进到真实源链/相位参考接入、正式可信度评分纳入和授权外部数据标定报告。",
    },
    {
        "阶段": "V4.0",
        "名称": "HPM 数字孪生平台",
