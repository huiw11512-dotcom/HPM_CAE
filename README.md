...
## V2.0 产品路线重定义：Scene First

V2.0 的主界面不再以“验证中心”作为默认入口，而是以“场景编辑”为首页。用户打开平台后应先看到阵列、目标、观察面、反射面、保护区和三维场分布，再进入求解、结果和论文导出。可信度验证仍保留，但作为 `工具 -> 可信度验证` 的高级复核入口，主要服务发文、报告和验收。
...
后续新增功能必须优先回答“用户现在能分析什么新的场景或任务”，而不是只回答“新增了哪个验证指标”。阶段路线调整为 V2.0B 三维场景编辑器（Scene First）、V2.0C 任务级仿真框架（Mission First）、V2.0D 论文自动生产线（Publication First）、V2.1 数字孪生任务库和 V3.0 真实数据接入与实验闭环。
...
- 平台成熟度已读取 Paper Factory 投稿准备度审计，把“投稿准备度审计”和“正式投稿门槛”加入主链路，并把 DOI、正式复现实验编号、PDF 归档和目标期刊模板签名缺口纳入关键阻断项；
...
- 前端默认首页已切换为“场景编辑”，并在首屏固定展示 `场景 / 对象 / 任务 / 结果` 四个核心概念；左侧主导航重排为项目管理、场景编辑、阵列设计、传播环境、求解设置、结果查看和论文导出，可信度验证降级为工具入口；
- 新增本地 Three.js 0.166.1 三维视口，不依赖公共 CDN；
- 新增 `/api/workbench3d/scene`、`/api/workbench3d/objects/{object_id}`、`/api/workbench3d/materials/{material_id}`、`/api/workbench3d/materials/audit`、`/api/workbench3d/absolute-calibration`、`/api/workbench3d/solve`、`/api/workbench3d/solve-jobs`、任务审计/暂停/恢复/取消/重试、`/api/workbench3d/results`、`/api/workbench3d/assets`、`/api/workbench3d/assets/audit`、`/api/workbench3d/assets/database`、`/api/workbench3d/assets/database/records`、`/api/workbench3d/assets/lineage`、`/api/workbench3d/assets/reproducibility`、`/api/workbench3d/assets/imported-calibration`、`/api/workbench3d/assets/naming`、`/api/workbench3d/reset`、撤销/重做、工程快照和快照差异 API；
...
- 新增 `/api/data-import/catalog`、`/api/data-import/acceptance`、`/api/data-import/calibration-readiness`、`/api/data-import/calibration-bridge`、`/api/data-import/model-comparison`、`/api/data-import/evidence-chain`、`/api/data-import/evidence-package/template`、`/api/data-import/evidence-package`、`/api/data-import/evidence-package/vv-candidate`、`/api/data-import/vv-audit`、`/api/data-import/samples/{sample_id}`、`/api/data-import/inspect`；
...
- 新增外部数据证据包 V&V 候选评分，支持把通过审计的授权证据包接入源链、相位参考、残差和 2σ 覆盖率门槛，输出 `evidence_package_vv_candidate.json`，但不自动改写正式可信度评分；
...
后续规划包括：补齐 V2.0B 真正三维拖拽式尺寸/旋转 Gizmo、工厂级资产/快照命名策略、正式多用户调度器和工程/结果数据库管理，扩展 V2.0C 插件级 V&V、外部插件签名/沙箱和保护/控场插件，补齐 V2.0D 外部 DOI 绑定、真实授权数据论文证据链、本机编译 PDF 归档和目标期刊模板插件生态，并把 V3.0 数据导入继续推进到真实源链/相位参考接入、正式可信度评分纳入和授权外部数据闭环标定报告。V4.0 面向实验室公共 HPM 数字孪生平台。
...
configs/external_data_evidence.yaml               外部数据证据链/相位参考配置
configs/paper_factory_v20d.yaml                   V2.0D 论文工厂配置
