...
PLATFORM_LAYER_STATUS = (
    ...
    },
    {
        ...
        "下一步": "补外部文献 DOI、正式复现实验编号、真实源链/相位参考和真实数据导入后的正式评分纳入。",
    },
    {
        "层级": "CAE平台层",
        "英文": "Workbench Layer",
        ...
        "当前证据": "已有 FastAPI + Bootstrap 工作台、V2.0B Three.js 本地三维视口、对象树、属性面板、材料设置、材料代理审计、拖拽移动、求解联动、观察面结果图层回写、色标与中心剖面读数、求解结果档案、轻量结果索引、撤销重做、工程快照落盘恢复、受控更新 API，以及 V2.0C 本地插件目录、启停、白名单运行和验收 API。",
...
PLATFORM_MILESTONE_STATUS = (
    ...
    },
    {
        ...
        "名称": "真正三维 CAE 编辑器",
        "状态": "进行中",
        "验收证据": "已接入 Three.js 本地视口、对象树、属性面板、材料设置、材料代理审计、拖拽移动、求解联动、观察面结果图层回写、色标与中心剖面读数、求解结果档案、轻量结果索引、撤销重做、工程快照落盘恢复和后端几何/材料校验 API。",
    ...
    },
    {
        ...
        "状态": "预览接入中",
        "验收证据": "已有本地 JSON manifest、语义版本校验、启停 API、参数 Schema、白名单 builtin_hook、插件市场页面、五个内置插件运行审计和论文模板插件协议。",
        "下一门槛": "补插件级 V&V、外部插件包签名/沙箱、保护/控场插件、外部目标期刊模板包和工作台求解器联动。",
    },
    {
        "阶段": "V2.0D",
