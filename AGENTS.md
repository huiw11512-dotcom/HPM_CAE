...
V2.0B 以后采用 Scene First，而不是 Verification First。默认首页必须是“场景编辑”，用户打开软件应先看到三维场景、阵列、目标、观察面、保护区、求解和结果；“可信度验证”只能作为 `工具 -> 可信度验证` 的高级复核入口。

后续新增功能必须优先回答“用户现在能分析什么新场景/任务”，再考虑验证指标、成熟度报表或后台审计。
...
- V2.0A：可信度验证体系 V&V。
- V2.0B：三维场景编辑器（Scene First），优先考虑 Three.js / VTK。
- V2.0C：任务级仿真框架（Mission First），插件系统与 Plugin Marketplace 作为基础设施。
- V2.0D：论文自动生产线（Publication First）。
- V2.1：数字孪生任务库。
- V3.0：真实数据接入与实验闭环，接入 CST、HFSS、测量数据、CSV、Touchstone、HDF5。
...
当前交付包处于 V2.0A 到 V2.0B 过渡阶段。V2.0A 是可信度层的阶段成果，不是平台最高目标；V2.0B 的首要纠偏是把首页改为 Scene First 三维场景工作台。后续开发不得把“开发某个 V2.x 功能”重新写成 North Star，也不得继续把默认入口设计成验证后台。
