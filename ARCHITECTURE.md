...
V&V 结果 -> 三维 Workbench 资产台账 -> 数据导入标定桥接 -> 插件目录 -> Paper Factory -> 平台成熟度与发文准备度报告。

`configs/platform_readiness.yaml` 管理成熟度权重、门槛和安全边界，`src/hpm_platform/readiness.py` 只执行配置化评估。
