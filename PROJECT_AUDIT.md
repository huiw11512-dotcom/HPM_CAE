...
| 禁止复制已有功能 | 主控台和成熟度报告复用 V&V、Workbench、DataImport、Plugin、PaperFactory 服务结果；证据包模板/审计/候选评分复用 V3.0 数据导入、证据链、Workbench 阵元功率元数据与外部 V&V 审计 |
...
- V3.0 已具备正式证据包模板、审计入口和 V&V 候选评分入口；但内置代理测量批次的残差和 2σ 覆盖率仍不满足正式评分门槛，仍缺可公开复查的高质量真实源链、相位参考和授权测量数据包。
