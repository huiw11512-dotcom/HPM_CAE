"""HPM-CAE V1.2 visual workbench.

The V1.2 UI adds object-level constrained multi-target field shaping, signed
constraint margins, and Pareto trade-off exploration while retaining the live
perception/protection chain and checkpointed queue. All quantities remain
normalized research proxies.
"""
...
UI_KEYS = (
    ...
    "solver_method", "iterations", "target_amplitude", "outside_penalty", "outside_hinge", "outside_peak_limit_db",
    "protected_penalty", "fairness_penalty", "tail_penalty", "tail_fraction", "pareto_points",
...
.v12-scope{border-left:4px solid #ffc857;background:#091422;border-radius:8px;padding:10px;color:#91a2bb;font-size:11px;line-height:1.6}.v12-live-banner{display:flex;gap:7px;flex-wrap:wrap}.v12-live-banner span{border:1px solid rgba(78,224,165,.35);background:rgba(78,224,165,.07);color:#4ee0a5;border-radius:999px;padding:5px 9px;font-size:10px}.queue-controls button{min-width:100px}.gradio-container table{font-size:11px!important}
"""
CSS = V10_CSS + EDITOR_CSS + EXTRA_CSS

HEADER = """
<div class="cae-topbar-inner"><div class="cae-brand"><div class="cae-logo">H</div><div><div class="cae-title">HPM-CAE Workbench <span style="color:#35d8ff">V1.2</span></div><div class="cae-sub">Object-constrained multi-target shaping · Pareto explorer · live full chain · resumable queue</div></div></div><div class="cae-badges"><span class="cae-badge">ALL PYTHON</span><span class="cae-badge">LIVE FULL CHAIN</span><span class="cae-badge">CONSTRAINED MULTI-OBJECT</span><span class="cae-badge">PARETO</span><span class="cae-badge">NORMALIZED MODE</span></div></div>
"""

PIPELINE = """
<div class="pipeline"><div class="stage ready"><b>① 信号/感知</b><span>相干多径 · PAWR实时计算</span></div><div class="arrow">›</div><div class="stage ready"><b>② 接收防护</b><span>DOA置信域 · 鲁棒宽零陷</span></div><div class="arrow">›</div><div class="stage ready"><b>③ 约束多目标控场</b><span>对象优先级 · 禁照上限 · PGMS</span></div><div class="arrow">›</div><div class="stage ready"><b>④ 约束 / Pareto</b><span>裕量图 · 折中前沿</span></div><div class="arrow">›</div><div class="stage ui"><b>⑤ CAE管理</b><span>对象树 · 批处理 · 报告</span></div></div>
"""
...
def _project_from_ui(state: dict, values: Sequence[Any], targets: Any, zones: Any, interferers: Any) -> CAEProject:
    ...
    project = replace(
        ...
        solver=replace(
            ...
            outside_peak_limit_db=float(v["outside_peak_limit_db"]), protected_penalty=float(v["protected_penalty"]), fairness_penalty=float(v["fairness_penalty"]),
            tail_penalty=float(v["tail_penalty"]), tail_fraction=float(v["tail_fraction"]),
            pareto_points=int(v["pareto_points"]),
...
def _ui_values(project: CAEProject) -> list[Any]:
    return [
        ...
        project.solver.method, project.solver.iterations, project.solver.target_amplitude, project.solver.outside_penalty, project.solver.outside_hinge_amplitude, project.solver.outside_peak_limit_db,
        project.solver.protected_penalty, project.solver.fairness_penalty, project.solver.tail_penalty, project.solver.tail_fraction, project.solver.pareto_points,
...
def _static_callback(state: dict, *args: Any):
    try:
        ...
        detail = (
            f"总体RMSE {result.metrics['target_rmse_percent']:.2f}% · "
            f"最差目标 {result.metrics['worst_target_rmse_percent']:.2f}% · "
            f"保护区最坏超限 {result.metrics['maximum_protected_violation_db']:+.2f} dB"
        )
        return (
            project.to_dict(), result, make_scene_figure(project), make_field_figure(result),
            make_constraint_margin_figure(result), make_object_metrics_figure(result),
            make_cut_figure(result), make_far_field_figure(result), make_weights_figure(result),
            make_convergence_figure(result), result.metrics_frame(), result.object_metrics_frame(),
            make_metric_cards_html(result), "\n".join(result.log_lines), str(report), str(archive),
            object_tree_html(project), _status("约束多目标静态求解完成", detail, "ok" if result.metrics["control_success"] else "warn"),
        )
    except Exception as exc:
        error = f"{type(exc).__name__}: {exc}"; empty = make_empty_result_figure("求解失败", error)
        return (
            state, None, empty, empty, empty, empty, empty, empty, empty, empty,
            pd.DataFrame([{"指标":"错误","数值":error}]), pd.DataFrame([{"错误":error}]),
            "", traceback.format_exc(), None, None, object_tree_html(CAEProject.from_dict(state)),
...
def _pareto_callback(state: dict, fast_mode: bool, *args: Any):
    try:
        ...
        folder, report, archive = export_pareto_bundle(study, PARETO_ROOT)
        record = study.recommended_record
        detail = (
            f"推荐 μ={float(record['risk_multiplier']):.3g} · "
            f"最差目标RMSE {float(record['worst_target_rmse_percent']):.2f}% · "
            f"风险超限 {float(record['risk_violation_db']):+.2f} dB"
        )
        return (
            project.to_dict(), study, make_pareto_figure(study), make_tradeoff_figure(study),
            make_pareto_field_gallery(study), study.records, "\n".join(study.log_lines),
            str(report), str(archive), _status("Pareto 扫描完成", detail, "ok"),
        )
    except Exception as exc:
        error = f"{type(exc).__name__}: {exc}"; empty = make_empty_result_figure("Pareto 扫描失败", error)
        return state, None, empty, empty, empty, pd.DataFrame([{"错误":error}]), traceback.format_exc(), None, None, _status("Pareto 扫描失败", error, "error")
...
def _initial_project() -> CAEProject:
    demo_path = PROJECT_ROOT / "configs" / "cae_project_v12.yaml"
    try:
        return CAEProject.load_yaml(demo_path) if demo_path.exists() else default_project()
    except Exception:
        return default_project()


def build_app() -> gr.Blocks:
    initial=_initial_project(); target_frame,zone_frame,interferer_frame=project_to_object_frames(initial)
    empty=make_empty_result_figure("尚未运行","从左侧同步场景，然后运行对应求解节点。")
    ui_values=_ui_values(initial)
    with gr.Blocks(title="HPM-CAE V1.2") as demo:
        project_state=gr.State(initial.to_dict()); static_state=gr.State(); pareto_state=gr.State(); perception_state=gr.State(); protection_state=gr.State(); workflow_state=gr.State(); timeline_state=gr.State(); job_state=gr.State("")
        ...
        with gr.Row(elem_id="workbench-row"):
            with gr.Column(scale=21,min_width=300,elem_classes="cae-sidebar"):
                ...
                with gr.Accordion("空间控场",open=False):
                    solver_method=gr.Dropdown(choices=list(initial.solver.METHODS),value=initial.solver.method,label="求解器"); iterations=gr.Slider(20,800,value=initial.solver.iterations,step=10,label="迭代"); target_amplitude=gr.Number(value=initial.solver.target_amplitude,label="目标归一化幅度")
                    with gr.Row(): outside_penalty=gr.Number(value=initial.solver.outside_penalty,label="区外惩罚"); outside_hinge=gr.Number(value=initial.solver.outside_hinge_amplitude,label="区外铰链幅度")
                    outside_peak_limit=gr.Number(value=initial.solver.outside_peak_limit_db,label="区外峰值上限 / dB")
                    with gr.Row(): protected_penalty=gr.Number(value=initial.solver.protected_penalty,label="保护区惩罚"); fairness_penalty=gr.Number(value=initial.solver.fairness_penalty,label="目标公平性惩罚")
                    with gr.Row(): tail_penalty=gr.Number(value=initial.solver.tail_penalty,label="尾部峰值惩罚"); tail_fraction=gr.Slider(0.05,0.40,value=initial.solver.tail_fraction,step=0.01,label="尾部比例")
                    pareto_points=gr.Slider(3,13,value=initial.solver.pareto_points,step=1,label="Pareto点数")
                ...
                sync_button=gr.Button("同步场景",variant="secondary"); status_html=gr.HTML(_status("V1.2工程就绪","对象级约束、多目标优化和Pareto模块已加载。"))
                gr.HTML('<div class="v12-scope"><b>模型边界</b><br>仅处理波长尺度几何、归一化复场、相对阵列响应与无量纲代理评价；不输出绝对源功率、具体器件阈值、现实毁伤概率或作用距离。</div>')
            with gr.Column(scale=58,min_width=680,elem_id="viewport-panel"):
                with gr.Tabs(elem_id="viewport-tabs"):
                    ...
                    with gr.Tab("静态约束控场"):
                        static_run=gr.Button("▶ 运行约束多目标静态求解",variant="primary")
                        static_cards=gr.HTML(''); field_plot=gr.Plot(empty,show_label=False)
                        with gr.Tabs():
                            with gr.Tab("约束裕量"): constraint_plot=gr.Plot(empty,show_label=False)
                            with gr.Tab("对象级指标"): object_metric_plot=gr.Plot(empty,show_label=False)
                        ...
                        with gr.Tabs():
                            with gr.Tab("总体指标"): static_metrics=gr.Dataframe(pd.DataFrame(),interactive=False,show_row_numbers=False)
                            with gr.Tab("对象明细"): object_metrics_table=gr.Dataframe(pd.DataFrame(),interactive=False,show_row_numbers=False)
                        static_log=gr.Code(value="",language="shell",lines=9,max_lines=24,interactive=False,show_line_numbers=False)
                    ...
                    with gr.Tab("Pareto 设计空间"):
                        gr.HTML('<div class="tab-note">扫描区外/保护区风险权重，自动识别非支配方案并推荐一个兼顾最差目标误差与约束风险的折中点。</div>')
                        with gr.Row(): pareto_fast=gr.Checkbox(value=True,label="快速交互模式"); pareto_run=gr.Button("▶ 运行 Pareto 扫描",variant="primary")
                        pareto_plot=gr.Plot(empty,show_label=False)
                        with gr.Tabs():
                            with gr.Tab("权重扫描"): tradeoff_plot=gr.Plot(empty,show_label=False)
                            with gr.Tab("代表场分布"): pareto_gallery=gr.Plot(empty,show_label=False)
                        pareto_records=gr.Dataframe(pd.DataFrame(),interactive=False,show_row_numbers=False,max_height=360)
                        pareto_log=gr.Code(value="",language="shell",lines=9,max_lines=24,interactive=False,show_line_numbers=False)
                        with gr.Row(): pareto_report=gr.DownloadButton("下载Pareto报告",value=None); pareto_archive=gr.DownloadButton("下载Pareto结果ZIP",value=None,variant="primary")
                    with gr.Tab("实时感知 / 防护"):
                        gr.HTML('<div class="v12-live-banner"><span>实时信号生成</span><span>PAWR / FBSS / ESPRIT</span><span>DOA不确定度</span><span>鲁棒宽零陷</span></div>')
                ...
                with gr.Accordion("Workbench Log",open=False): global_note=gr.Markdown("V1.2 constrained multi-object chain ready.")

        components=[project_name,seed,frequency,nx,ny,spacing_x,spacing_y,z_lambda,span_x,span_y,samples,solver_method,iterations,target_amplitude,outside_penalty,outside_hinge,outside_peak_limit,protected_penalty,fairness_penalty,tail_penalty,tail_fraction,pareto_points,uncertainty_scenarios,gain_std,phase_std,registration_jitter,pa_enabled,dpd_enabled,perception_method,snr_db,snapshots,fault_count,prior_sigma,prior_strength,protection_method,desired_theta,desired_phi,interferer_power,sector_scale,soft_strength,parallel_workers]
        ...
        static_run.click(_static_callback,inputs=common_inputs,outputs=[project_state,static_state,scene_plot,field_plot,constraint_plot,object_metric_plot,cut_plot,far_plot,weights_plot,convergence_plot,static_metrics,object_metrics_table,static_cards,static_log,static_report,static_archive,object_tree,status_html],show_progress="full",concurrency_limit=1)
        pareto_run.click(_pareto_callback,inputs=[project_state,pareto_fast,*components,target_table,zone_table,interferer_table],outputs=[project_state,pareto_state,pareto_plot,tradeoff_plot,pareto_gallery,pareto_records,pareto_log,pareto_report,pareto_archive,status_html],show_progress="full",concurrency_limit=1)
